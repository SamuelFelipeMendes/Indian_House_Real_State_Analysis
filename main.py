...
def generic(data):
    variaveis=data['total_sqm','room','bath']
    # Calcular correlação das variáveis com Preço
    correlacoes = data.corr(numeric_only=True)["Preço"].index()
    correlacoes.columns = ["variaveis", "Correlação"]

    # Gráfico de barras
    plt.figure(figsize=(6, 4))
    sns.barplot(data=correlacoes, x="Variável", y="Correlação", palette="Blues_d")
    plt.title("Correlação das Variáveis com o Preço")
    plt.ylim(-1, 1)  # limite de correlação
    plt.show()
...
def boxplot_room_price(data):
    sns.boxplot(data=data, x="area_type", y="Price_per_M²")
